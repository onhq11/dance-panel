...
export const GET = async () => {
  try {
    const [rows] = await db.query(
      `SELECT age_group.id, age_group.name, age_group.seats - (SELECT COUNT(*) FROM participant WHERE age_group_id = age_group.id) as available_seats FROM age_group WHERE is_solo = 1`,
    );
